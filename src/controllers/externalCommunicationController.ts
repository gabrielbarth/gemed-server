...
class ExternalCommunicationController {
  async store(request: Request, response: Response) {
    try {
      ...
      const communication = await ExternalCommunication.create(request.body);
  ...
  async update(request: Request, response: Response) {
    try {
      ...
      const communication = await ExternalCommunication.findByIdAndUpdate(request.params.communicationId, {
        title, 
        to,
        author
      }, { new: true });
  ...
  async delete(request: Request, response: Response) {
    try {
      await ExternalCommunication.findByIdAndRemove(request.params.communicationId);
